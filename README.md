...
4. (Optional) `data/statnuri_unsold_panel.parquet`, `data/statnuri_completed_unsold_panel.parquet`, `data/national_redevelopment_intensity.parquet`, `data/statnuri_landuse_panel.parquet`, `data/rtms_rent_panel.parquet` — Block 4 controls (pre-completion unsold, post-completion unsold, redev intensity, gu-level land-use context) and Block 1 multi-housing tenure panel (apt + rowhouse/multifamily + single/multi-generation + officetel; produced by `build_rtms_rent_panel.py` from the four per-type sibling panels).
...
- **Block 1 — Tenure pressure (RTMS multi-housing).** RESOLVED 2026-06-10 for all four data.go.kr RTMS rent endpoints — apartment (15126474), 연립다세대 (15126473), 단독/다가구 (15126472), and 오피스텔 (15126475). The dashboard contract flips `tenure_status` from `missing_local_artifact` to `live` when `data/rtms_rent_panel.parquet` is present (the combined four-type panel produced by `build_rtms_rent_panel.py`). The 9,600-row panel covers 25 Seoul gus × 8 years × 12 months × 4 housing types. Per-type wolse_ratio rollups are exposed (`tenure_wolse_ratio_apt`, `_rowhouse_multifamily`, `_single_detached`, `_officetel`) plus an `_all_residential` sum-based rollup. Per-m² medians are exposed for apt / RH / Offi only; **single_detached carries NaN per-m²** because its endpoint (SHRent) only exposes `totalFloorAr` (whole-building gross), which is not comparable to per-unit `excluUseAr` from the other three types — see `docs/rtms_siblings_probe_2026-06-09.md`. Annual ratios are recomputed from annual sums per housing type, never averaged from monthly ratios.
- **Block 3 — Vulnerability.** Scoped 2026-06-11 via `docs/vulnerability_source_status.md`. v1 plan: four descriptive indicators at gu-year broadcast (mirroring Block 4b/4c convention) — `senior_population_share`, `single_person_household_share`, `foreign_resident_share`, `basic_livelihood_recipient_share`. Sources are KOSIS / Seoul Open Data / 행정안전부 — all public, no MOLIT approval required. Unblocks on: free KOSIS or SGIS OpenAPI key + portal-side schema browse to pin (stat_id, table_id) values for the four indicators. Each indicator measures one underlying construct and is exposed as its own column; no composite, no score, no classification.
...
- **MOLIT 통계누리 시·군·구별 미분양현황** (form_id 2082, style_num 128), pulled monthly by `molit_unsold_client.py` over 2017-01 – 2024-12 and aggregated to annual at gu grain into `data/statnuri_unsold_panel.parquet`. Seoul gus only at present; nationwide extension requires province-disambiguated gu-name → LAWD_CD mapping. API quirk: when monthly unsold = 0 the field `미분양현황` is omitted from the row; the builder treats omitted-field as zero per the empirical convention (verified against 2020 vs 2024 cache files). `MOLIT_UNSOLD_FORM_ID`/`_STYLE_NUM` env vars override defaults. **Pre-completion / pre-sale unsold inventory.**
- **MOLIT 통계누리 공사완료후 미분양현황** (form_id 5328, style_num 1), pulled monthly by `molit_completed_unsold_client.py` over 2017-01 – 2024-12 and aggregated to annual at gu grain into `data/statnuri_completed_unsold_panel.parquet`. **Post-completion unsold inventory** — the canonical 'overhang' indicator. Sits alongside the pre-completion unsold panel as a second Block 4b sub-row. Schema returns one row per `(gu, month, 규모, 부문)`; the builder filters to `규모=계 AND 부문=계` to get the canonical per-gu-month total. Opposite zero-handling to form 2082/128: rows with value 0 are PRESENT with explicit `호=0`, so missing values after the merge are true join-failure NAs, not legitimate zeros. `MOLIT_COMPLETED_UNSOLD_FORM_ID`/`_STYLE_NUM` env vars override defaults.
...
- **MOLIT RTMS 전월세 실거래가 — multi-housing** (data.go.kr OpenAPI; four sibling endpoints `RTMSDataSvcAptRent`/15126474, `RTMSDataSvcRHRent`/15126473, `RTMSDataSvcSHRent`/15126472, `RTMSDataSvcOffiRent`/15126475). Per-housing-type panels are built by `molit_client.py build-panel --housing-type X`; the combined panel `data/rtms_rent_panel.parquet` (9,600 gu-month rows = 25 Seoul gus × 8 yrs × 12 mo × 4 housing types) is assembled by `build_rtms_rent_panel.py combine`. All four endpoints respond on the same URL convention, accept `resultCode` "00" or "000", and need `numOfRows=1000` to single-shot a typical urban gu-month. Classification rule: `monthlyRent == 0 or blank → jeonse`; otherwise `wolse`. Per-m² metric semantics differ across siblings: apt/RH/Offi expose `excluUseAr` (per-unit exclusive area) and per-m² medians are computed; SH exposes only `totalFloorAr` (whole-building gross) and per-m² medians are emitted as NaN by construction — see `docs/rtms_siblings_probe_2026-06-09.md` for the schema gap. Block 1 (tenure pressure) consumes this panel as a **gu-level broadcast / context** layer with `tenure_scope='apt + rowhouse_multifamily + single_detached + officetel'` and `tenure_status='live'`. `MOLIT_SERVICE_KEY` env var must hold the **decoded** key from data.go.kr 마이페이지.
...
| Gu-level pre-completion unsold-housing stress control (Block 4b sub-row 1) | implemented; 96 monthly pulls (2017-01..2024-12) over Seoul's 25 gus; annual mean/max/Dec; joined into `data/dong_year_model_panel.parquet` by `lawd_cd × year`. Form 2082/128. |
| Gu-level post-completion unsold control (Block 4b sub-row 2) | implemented as `molit_completed_unsold_client.py`; 96 monthly pulls (2017-01..2024-12) over Seoul's 25 gus; annual mean/max/Dec written to `data/statnuri_completed_unsold_panel.parquet`. Form 5328/1. Joins into the dashboard contract as `statnuri_completed_unsold_{mean,max,dec}_units` under separate `completed_unsold_status` to avoid conflating 'inventory waiting to sell' (4b sub-row 1) with 'inventory built but unsold' (4b sub-row 2). |
...
| Live multi-housing tenure panel (Block 1) | implemented as `molit_client.build_seoul_tenure_panel(housing_type, ...)` per housing type + `build_rtms_rent_panel.py` combiner. 9,600 monthly pulls over 25 Seoul gus × 8 years × 12 months × 4 housing types = 9,600 gu-month rows in `data/rtms_rent_panel.parquet`. Six core metrics per gu-month-type plus a `housing_type` dimension. Merges into the dashboard contract as **RTMS multi-housing gu-year broadcast** with `tenure_scope='apt + rowhouse_multifamily + single_detached + officetel'` and `tenure_status='live'`. Five wolse_ratio rollups exposed (all-residential + per-type, all recomputed from annual sums per type). Per-m² medians exposed for apt / RH / Offi; SH carries NaN per-m² by construction (totalFloorAr vs excluUseAr semantic gap). |
