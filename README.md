...
4. (Optional) `data/statnuri_unsold_panel.parquet`, `data/statnuri_completed_unsold_panel.parquet`, `data/national_redevelopment_intensity.parquet`, `data/statnuri_landuse_panel.parquet`, `data/wolse_molit.parquet` — Block 4 controls (pre-completion unsold, post-completion unsold, redev intensity, gu-level land-use context) and Block 1 apartment-only tenure panel.
...
- **Block 1 — Tenure pressure (apartment-only).** RESOLVED 2026-06-09 for the apartment-rent sub-scope via `RTMSDataSvcAptRent` (`data.go.kr` dataset 15126474). The dashboard contract flips `tenure_status` from `missing_local_artifact` to `live_partial` when `data/wolse_molit.parquet` is present. The "partial" qualifier is load-bearing: single/multi-family rent (`RTMSDataSvcSingleHouseRent`, dataset 15126472) and officetel rent are NOT yet integrated; their sibling endpoints share the same credential family but each needs its own client+builder before the scope can move from `apartment_only` to all-tenure. See `docs/tenure_source_status.md`.
...
- **MOLIT 아파트 전월세 실거래가** (data.go.kr OpenAPI `RTMSDataSvcAptRent`, dataset 15126474), pulled monthly by `molit_client.py build-panel` over 2017-01 – 2024-12 across all 25 Seoul gus into `data/wolse_molit.parquet` (gu × month grain). The endpoint paginates at default size 10; the client now sends `numOfRows=1000` and accepts `resultCode` "00" OR "000" (verified live 2026-06-08, 마포구 January 2024 returned exactly 1,137 of 1,137 transactions). Classification rule: `monthlyRent == 0 or blank → jeonse`; otherwise `wolse`. Per-m² metrics use `excluUseAr`. Six core metrics per gu-month: `n_rent_deals`, `n_wolse`, `n_jeonse`, `wolse_ratio`, `median_deposit_per_m2`, `median_monthly_rent_per_m2`. Block 1 (tenure pressure) consumes this panel as a **gu-level broadcast / context** layer with `tenure_scope=apartment_only` and `tenure_status=live_partial` — partial because single/multi-family and officetel rent flow through sibling endpoints not yet integrated. `MOLIT_SERVICE_KEY` env var must hold the **decoded** key from data.go.kr 마이페이지.
...
| Live apartment tenure panel (Block 1) | implemented as `molit_client.build_seoul_tenure_panel`; 2,400 monthly pulls (2017-01..2024-12) over Seoul's 25 gus = 2,400 gu-month rows in `data/wolse_molit.parquet`. Six core metrics per gu-month: n_rent_deals, n_wolse, n_jeonse, wolse_ratio, median_deposit_per_m2, median_monthly_rent_per_m2. Merges into the dashboard contract as **apartment-only gu-year broadcast** with `tenure_scope='apartment_only'` and `tenure_status='live_partial'`. Pilot signal verified: 마포구 wolse_ratio rose 35%→47% across 2017–2024, with a 2019→2020 step (30%→43%) tracking the COVID/rate-hike inflection. |
