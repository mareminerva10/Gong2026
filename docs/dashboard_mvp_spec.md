...
|---|---|---|---|---|
| 1 Tenure pressure (RTMS multi-housing) | counts (all-residential annual sums): `tenure_n_rent_deals`, `tenure_n_wolse`, `tenure_n_jeonse`; wolse_ratio rollups: `tenure_wolse_ratio_all_residential`, `tenure_wolse_ratio_apt`, `tenure_wolse_ratio_rowhouse_multifamily`, `tenure_wolse_ratio_single_detached`, `tenure_wolse_ratio_officetel`; per-m² medians (apt/RH/Offi only): `tenure_median_deposit_per_m2_*`, `tenure_median_monthly_rent_per_m2_*` | data.go.kr RTMS: `RTMSDataSvcAptRent` (15126474), `RTMSDataSvcRHRent` (15126473), `RTMSDataSvcSHRent` (15126472), `RTMSDataSvcOffiRent` (15126475) | gu × month × housing_type, annualized to gu × year × housing_type, then broadcast onto dong × year | **LIVE** — resolved via `build_rtms_rent_panel.py` against `molit_client.HOUSING_TYPE_REGISTRY`. tenure_scope = 'apt + rowhouse_multifamily + single_detached + officetel', tenure_status = 'live'. Annual ratios recomputed from annual sums per housing type, never averaged from monthly ratios. SH per-m² is NaN by construction (totalFloorAr vs excluUseAr semantic gap; see `docs/rtms_siblings_probe_2026-06-09.md`). |
...
| 3 Vulnerability | v1 set: `senior_population_share`, `single_person_household_share`, `foreign_resident_share`, `basic_livelihood_recipient_share` | KOSIS / Seoul Open Data / 행정안전부 (per indicator; see `docs/vulnerability_source_status.md`) | gu × year, broadcast onto dong × year | **SCOPED, NOT INTEGRATED** (2026-06-11) — v1 plan locked at gu-year broadcast; integration blocked on free KOSIS/SGIS OpenAPI key + portal-side (stat_id, table_id) discovery. See `docs/vulnerability_source_status.md` and [B2] below. |
...
|---|---|---|---|
| B1 | 전월세 source — all four RTMS rent endpoints integrated (apt + 연립다세대 + 단독/다가구 + 오피스텔) | RESOLVED (RTMS multi-housing) | Block 1 live at gu-year broadcast; promotion of tenure beyond `live` would require non-tenure signals (e.g. household income, displacement panels), not a broader RTMS scope. SH per-m² is structurally NaN due to the SHRent `totalFloorAr` vs `excluUseAr` semantic gap — a future SH-specific `*_per_total_floor_m2` family could fill this, but the current Block 1 surface is the minimal MVP set. |
| B2 | Vulnerability block — v1 source candidate set identified at gu-year grain (4 indicators across KOSIS / Seoul Open Data / 행정안전부). Integration still requires a free KOSIS/SGIS OpenAPI key + portal-side (stat_id, table_id) discovery for each indicator. | RESOLVED 2026-06-11 (scoping); INTEGRATION pending user-side actions in `docs/vulnerability_source_status.md` §7 | Block 3 unparked = `vulnerability_status` flips from `not_scoped` to `live` after the four indicators integrate via the same gu-broadcast pattern as Block 4b/4c. |
