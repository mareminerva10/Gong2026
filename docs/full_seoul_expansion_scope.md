...
- **#3 — mid-write atomicity.** The criterion read "kill the process mid-pull, restart, confirm the resume picks up only uncached (poly, year) pairs." What `ae08346` empirically tested is **clean-stop resume** (an `--offline` rerun reported 24/24 cached after a bounded `--limit 24` run), not a SIGKILL mid-write. `seoul_pilot_extract.py` writes `pd.DataFrame([rec]).to_parquet(cache, index=False)` directly to the final cache path; a kill during the flush would leave a truncated parquet there that the next run's `cache.exists()` check would accept as cached and then fail to parse. Bounded clean restarts cover the common case, but the strict criterion is not met. *Suggested follow-up*: write to `cache.with_suffix(".tmp")` and `tmp.replace(cache)` so the read-side never sees a partial file.
...
- **#7 — partial cost record.** §8 asked for "seconds/minutes per polygon-year for both per-call and (if attempted) batch reductions." What exists is aggregate wall-clock from bounded smokes (~0.8–1.0 s per fresh pull at steady state, per `ae08346`), printed by `seoul_pilot_extract.py` but not persisted; no per-(emd_cd, year) timing log; the full 320-row pilot run's aggregate wall-clock is not recorded in any committed doc. Batch reductions were intentionally not attempted (consistent with the per-call pilot plan, deferred to full-Seoul). *Suggested follow-up*: extend `fetch_embeddings` to append one row per fresh pull to `data/seoul_pilot_cost_log.csv` with `(emd_cd, year, elapsed_s)`; the full-Seoul cost estimate then comes from a one-line aggregate over that log rather than from prose in the docs.
