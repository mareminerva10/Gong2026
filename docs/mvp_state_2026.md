...
- **Block 1 tenure pressure (RTMS multi-housing) live**. Combined panel `data/rtms_rent_panel.parquet` (9,600 gu-month rows = 25 Seoul gus × 8 years × 12 months × 4 housing types) built by `build_rtms_rent_panel.py` from the four `molit_client.build_seoul_tenure_panel(housing_type, ...)` per-type panels (apt: 15126474; rowhouse_multifamily: 15126473; single_detached: 15126472; officetel: 15126475). Dashboard contract exposes five wolse_ratio rollups (`tenure_wolse_ratio_all_residential` + four per-type), three all-residential annual counts (`tenure_n_rent_deals`, `tenure_n_wolse`, `tenure_n_jeonse`), and per-m² medians for apt / RH / Offi only. `tenure_scope = 'apt + rowhouse_multifamily + single_detached + officetel'`, `tenure_status = 'live'`, `tenure_grain = 'gu-year'`. Annual ratios are recomputed from annual sums per housing type, never averaged from monthly ratios. Single-detached carries NaN per-m² because the SHRent endpoint exposes `totalFloorAr` (whole-building gross) instead of `excluUseAr` (per-unit) — see `docs/rtms_siblings_probe_2026-06-09.md`.
