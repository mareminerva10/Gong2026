...
- **Block 4 controls live**: `statnuri_unsold_{mean,max,dec}_units` at gu × year grain via `molit_unsold_client.py`; `national_redevelopment_intensity_*` at national × year grain via `molit_redev_client.py`. Both join into the contract.
...
- **Block 1 (tenure pressure / wolse_ratio)** — parked. StatNuri keyword discovery surfaced no validated candidate (hRsId=37 rejected as supply/inventory-only, hRsId=24 rejected as land-use, hRsId=487 inconclusive). data.go.kr `RTMSDataSvcAptRent` is code-ready (`molit_client.py`) but credential-blocked — previous 401 was the StatNuri key in the data.go.kr slot. See `docs/tenure_source_status.md`. Unblocks on: portal-authenticated StatNuri catalog browse identifying a tenure-split form_id, OR a decoded `RTMSDataSvcAptRent` key in `MOLIT_SERVICE_KEY`.
...
- **Block 4c (spatial development companion)** — scouted but not unparked. Track 1 (의제처리구역 SHP) downloadable but governing license is **KOGL-4** at the Seoul Open Data file source (no commercial use / no derivatives), incompatible with derived `agenda_zone_*` dashboard columns. Track 2 (Seoul aggregate tables 235 / 10804 / 145) shows clean KOGL-1 metadata but file-download slots are empty. 건축HUB OpenAPI parked as post-MVP. See `docs/development_spatial_companion_status.md` §6–§8. Unblocks on: KOGL-1 license clarification or substitute source for the designation overlay, OR a non-file access path (CSV/API/stat-table) for the Seoul aggregate tables.
